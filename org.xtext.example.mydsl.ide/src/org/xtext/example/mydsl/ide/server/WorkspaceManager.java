...
public class WorkspaceManager {
	...
	public Buildable didChangingFiles(List<URI> dirtyFiles, List<URI> deletedFiles) {
		BuildManager.Buildable buildable = buildManager.dryRun(dirtyFiles, deletedFiles);
		return (cancelIndicator) -> {
			List<IResourceDescription.Delta> deltas = buildable.build(cancelIndicator);
			afterBuild(deltas);
			return deltas;
	...
	public BuildManager.Buildable didChangeTextDocumentContent(URI uri, Integer version,
		...
		return didChangingFiles(ImmutableList.of(uri), Collections.emptyList());
	}
